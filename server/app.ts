...
app.post('/api/members', async (req, res) => {
  try {
    const validatedMember = memberSchemaValidation.parse(req.body);
    await connectDB();
...
app.patch('/api/members/:id', async (req, res) => {
  try {
    ...
    if (oldMember && req.body.paidAmount > oldMember.paidAmount) {
      await MemberPaymentRecord.create({
        memberId: updatedMember._id,
        memberName: updatedMember.name,
        gymId: updatedMember.gymId,
        amount: req.body.paidAmount - oldMember.paidAmount,
        paymentDate: new Date().toISOString().split('T')[0],
        note: req.body.paidAmount >= updatedMember.feesAmount ? 'Balance Cleared' : 'Partial Payment'
      });
    }
